        ...
        <div class="dates-events">
          <div class="events-week" v-for="week in currentDates">
            <div class="events-day" v-for="day in week" track-by="$index" :class="{'today' : day.isToday,
              ...
              <!--<p class="day-number">{{day.monthDay}}</p>-->
              <div class="event-box" :class="[day.events[day.events.length -1]]">
                <p v-if="day.events.length > eventLimit" class="more-link" @click.stop="selectThisDay(day, $event)" :class="{'isSpecial' : day.events[day.events.length -1].distributor_id}">
                  <strong>结算价：</strong>{{day.events[day.events.length -1].supplier_settle_price}}<br>
                  <strong>分销价：</strong>{{day.events[day.events.length -1].distributor_settle_price}}<br>
                  <strong>售卖价：</strong>{{day.events[day.events.length -1].sales_price}}<br>
                  <strong>库存：</strong>{{day.events[day.events.length -1].share_stock}}<br>
                </p>
              </div>
            </div>
          </div>
